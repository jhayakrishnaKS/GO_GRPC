...
.DEFAULT_GOAL := help
.PHONY: greet blog calculator office help
project := greet calculator blog office
...
office: $@  ## Generate Pbs and build for office
...
clean: clean_greet clean_calculator clean_blog  clean_office## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_office: ## Clean generated files for office
	${RM_F_CMD} office/${PROTO_DIR}/*.pb.go
